...
namespace ProtocolPack
{
    ...
    public class PrtDCS
    {
        ...
        public Int16[] Values;
        //----------------------------------------------------------------------------------------------
        public PrtDCS(Byte[] Buf)
        {
            UnPack(Buf, Buf.Length);
        }
        //----------------------------------------------------------------------------------------------
        public Byte[] GetSendBuf(Int32 ID, Int16[] ValueList)
        {
        ...
        private int UnPack(Byte[] Buf,  int Len)
        {
            ...
            Pos += 2;
            //获取数据包内容
            ObjID = BitConverter.ToUInt32(Buf, Pos);        //对象ID
            Pos += 4;
            ...
            {
                Values[i] = BitConverter.ToInt16(Buf, Pos);        //数据值列表
                Pos += 2;
            }
            Receive(ObjID, Values);
            //
