    ...
    public partial class FrmMain : Form
    {
        ...
        public FrmMain()
        {
            try
            {
              ...
              // MR_CMS.Initialize();
              // MR_IHE.Start(3500); // 启动Mindray IHE协议监听
        ...
        void ViewMonitor_addAlarmRecord(AlarmRecord CurAlarmRecord)
        {
            ...
            string strSql = string.Format(@"insert into AlarmRecord(DeviceID,DeviceSN,AlarmType,BedNum,PatientName,Level,Val,ParaName,High,Low,AlarmTime)
                    ...
                    CurAlarmRecord.PatientName, CurAlarmRecord.Level, CurAlarmRecord.Val, CurAlarmRecord.ParaName, CurAlarmRecord.High, CurAlarmRecord.Low, CurAlarmRecord.AlarmTime.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        private void ShowSystemInfo()
        {
            try
            {
                this.Text = Config.Get("Sys", "Hospital", "") + " " + Config.Get("Sys", "Department", "") + Config.Get("Sys", "Name", " 中央监护系统");
            }
            catch (Exception ex)
            {
                Log.d(ex.Message); 
        ...
        private void btnCascadeSetting_Click(object sender, EventArgs e)
        {
            FrmCascadeSetting frmCascade = new FrmCascadeSetting();
            frmCascade.Owner = this;
            frmCascade.StartPosition = FormStartPosition.CenterScreen;
            if (frmCascade.ShowDialog() == DialogResult.OK) { }
        }
